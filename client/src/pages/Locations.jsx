...
import { getAllLocations } from '../services/LocationsAPI'; // Import API call
...
const Locations = () => {
    ...
    useEffect(() => {
        if (locations.length > 0) {
            setListeners(); // Set listeners only after locations are loaded
        }
    }, [locations]);
    ...
    return (
        <div className="available-locations">
            ...
            >
                ...
                {/* Polygons with dynamic href using location IDs */}
                <Link to={`/locations/${locations[0]?.id}/events`}>
                ...
                <Link to={`/locations/${locations[1]?.id}/events`}>
                    <polygon id="venue2" points="358.58,353.74 376.65,322.77 389.55,314.52 384.39,280.45 407.61,272.19 422.06,220.58 438.58,126.65 449.42,38.39 457.68,16.71 468,35.81 474.19,103.42 491.74,203.03 508.26,261.87 517.03,281.48 517.03,214.9 529.42,194.26 540.77,197.35 540.77,169.48 552.13,167.94 556.77,149.87 566.06,156.06 566.06,193.74 577.42,211.81 577.42,238.65 601.16,254.65 594.45,302.13 575.87,335.68 587.23,353.74 601.16,363.55 358.58,363.55" />
                </Link>

                <Link to={`/locations/${locations[2]?.id}/events`}>
                    <polygon id="venue3" points="998.06,83.81 952.65,31.16 914.45,16.71 877.29,43.55 833.94,102.39 811.74,161.23 796.77,241.23 802.97,303.16 833.94,353.23 871.61,385.23 954.71,385.23 1000.32,387.81" />
                </Link>

                <Link to={`/locations/${locations[3]?.id}/events`}>
                    <polygon id="venue4" points="625,291 615,305 608,318 625,338 637,354 622.5,358 673,363.5 751,363.5 793,363.5 769,352 772,347 793,340 806,321 796.8,291 784,269 757,261 730,272 707,281 672,283" />
                </Link>
            </svg>
        </div>
    );
